...
def pipeline_data_with_audio_paths(tmp_path):
    ...
    data = PipelineData(
        audio_intro_path=str(intro),
        active_file_path=str(main),
        audio_outro_path=str(outro),
    )
    return data
...
def test_merge_step_success(
    ...
    # Act
    result = merge_step(data=data, is_audio=True, output_format=output_format)
...
def test_merge_step_normalize_permission_error(
    ...
    with pytest.raises(PermissionError, match="Permission denied"):
        merge_step(data=data, is_audio=True, output_format="mp3")
